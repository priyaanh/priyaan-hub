...
**Free ChatGPT (the default, no key, no cost).** The page writes the prompt for you: press **Copy the prompt**, open ChatGPT, attach the PDF, paste, send, then paste its whole reply back into the box. The page reads the JSON it returns - or, failing that, a plain numbered list - and builds the same worksheet. Nothing is sent anywhere by the hub itself in this mode.
