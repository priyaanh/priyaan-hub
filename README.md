...
| 📄 From a PDF | `ai.html` | Drop in a chapter, worksheet or study guide and Claude or ChatGPT writes fresh practice problems or a multiple-choice quiz from it. Print the worksheet with its answer key, or answer the quiz on screen. Needs your own API key for whichever service you pick. |
...
Pick either **Claude** or **ChatGPT** at the top of the page - whichever account you already have - and choose a model (the dropdown shows what each costs). It calls that API straight from the browser, so it needs that service's API key, entered once on the page and kept in that browser's local storage only. There is no key-free option: both companies charge per use, and a static page has nowhere to hide a shared key. It is never committed, never included in an export, and never sent anywhere except to Anthropic. Get one at [console.anthropic.com](https://console.anthropic.com/), set a monthly spending limit, and expect a few cents per worksheet - the exact cost of each run is shown when it finishes. Keys are stored per service, so you can keep both. The page uses a JSON schema (Claude's `output_config.format`, OpenAI's `text.format`) so questions always come back in a fixed shape.
